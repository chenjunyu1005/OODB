...
public class Laptop extends Product
{
...

	{
		...
		q.setFilter("this.modelName==modelName");
		Collection<Laptop> result =(Collection<Laptop>)q.execute(mName);
		Laptop l =Utility.extract(result);
		q.close(result);
		return l;
	...
	{		q.setClass(Laptop.class);
	        q.declareParameters("boolean HDScreen,int driveCapacity");
	        q.setFilter("this.hasHDScreen == HDScreen && this.hardDriveCapacity>=driveCapacity");
	        q.setOrdering("this.hardDriveCapacity ascending,this.modelName ascending");
	        return(Collection<Laptop>)q.execute(true,x);
...

	{
		...
		q.setFilter("this.processor.madeBy.name == companyName");
